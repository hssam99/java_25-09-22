...
//		System.out.println(is.count()); // 에러, 소모되어서 더 사용 불가
		...
		// 배열로 리턴
		int arrFilter[] = Arrays.stream(arr).filter(n->n>5).toArray();
		System.out.println(Arrays.toString(arrFilter));
		
		// list로 filter
		List<Integer> list = Arrays.stream(arr).filter(n -> n>=5).
				boxed(). // IntStream > Stream<Integer>
				collect(Collectors.toList()); // collect(Collectors.toList())는 Stream의 결과를 List로 모으는 기능
		System.out.println(list);
		
		
		boolean any = Arrays.stream(arr).noneMatch(n->n==5); // 5랑 매치하는게 하나도 없음?
